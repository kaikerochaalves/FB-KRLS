...
l_M = [25, 50, 75, 100, 150, 200, 250, 300]
l_sigma = [0.01, 0.05, 0.1, 0.25, 0.5, 0.75, 1, 5, 10]
l_lambda1 = [1e-6, 1e-4, 1e-2]
...
columns = ['M', 'sigma', 'lambda1', 'RMSE', 'NDEI', 'MAE']
result = pd.DataFrame(columns = columns)
for M in l_M:
    for sigma in l_sigma:
        for lambda1 in l_lambda1:
                            
            print(f"M = {M} and sigma = {sigma} and lambda1 = {lambda1}")
        
            # Initializing the model
            model = FB_KRLS(M = M, sigma = sigma, lambda1 = lambda1)
            # Train the model
            ...
            NewRow = pd.DataFrame([[M, sigma, lambda1, RMSE, NDEI, MAE]], columns = columns)
