...
class FB_KRLS:
    def __init__(self, M = 30, sigma = 0.5, lambda1 = 0.1):
        ...
        # Computing the residual square in the training phase
    ...
    def Initialize_FB_KRLS(self, x, y):
        Kinv = np.eye(1) / (1 + self.lambda1)
        alpha = Kinv * y
        yn = np.eye(1) * y
    ...
    def FB_KRLS(self, x, y):
        ...
        if self.parameters.loc[i, 'Dict'].shape[1] > self.M:
            ...
            # Update yn
            self.parameters.at[i, 'yn'] = np.delete(self.parameters.loc[i, 'yn'], ind, 0)
            # Update k
            k = np.delete(k, ind)
            ...
            # Compute Dinv
            G = self.parameters.loc[i, 'Kinv'][noind, :][:, noind]
            f = self.parameters.loc[i, 'Kinv'][noind, ind].reshape(-1,1)
            ft = self.parameters.loc[i, 'Kinv'][ind,noind].reshape(1,-1)
            e = self.parameters.loc[i, 'Kinv'][ind,ind]
            D_inv = G - ( f @ ft ) / e
            # Update Kinv
